...
use RTSP::Utility qw($CRLF get_named_params invoke_callback);
...
The constructor method. This creates a new B<RTSP::Request> object and returns
a reference to it. The first and only mandatory argument is a string containing
...
The URI (alternately spelled URL) of the RTSP request. The request URI of an
RTSP request, if its present, must be absolute. Defaults to "*".
...
(Optional.) A hash reference containing header name/value pairs.
...
sub new
{
	...
	my ($uri, $url, $version, $headers, $body,
	    $success_callback, $failure_callback,
	    $session_success_callback, $session_failure_callback);

	$uri = shift if (scalar @_ % 2);
	...
	get_named_params({
		...
		SessionSuccessCallback => \$session_success_callback,
		SessionFailureCallback => \$session_failure_callback,
		}, \@_
	);
	...
	# the callback to invoke if the request is unsuccessful:
	$self->{failure_callback}         = $failure_callback;

	# the callback to invoke for session requests if the request is
	# successful:
	$self->{session_success_callback} = $session_success_callback;

	# the callback to invoke for session requests if the request is
	# unsuccessful:
	$self->{session_failure_callback} = $session_failure_callback;
...
sub session_success_callback
{
	my $self = shift;

	$self->{session_success_callback} = shift if @_;

	return $self->{session_success_callback};
}
...
sub session_failure_callback
{
	my $self = shift;

	$self->{session_failure_callback} = shift if @_;

	return $self->{session_failure_callback};
}
...
sub succeed
{
	...
	invoke_callback($self->session_success_callback, @_);
}
...
sub fail
{
	...
	invoke_callback($self->session_failure_callback, @_);
}
