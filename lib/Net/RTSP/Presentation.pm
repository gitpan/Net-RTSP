...
use RTSP::Utility qw(get_named_params);
...
sub new
{
	...
	my ($success_callback, $failure_callback,
	...
	my $self = {
		...
		server_request_callback  => $request_callback,
		...
		# the queue of pending request objects to dispatch for the
...
sub setup_session
{
	...
	my $session = new Net::RTSP::Session (@_);
...
sub send_request
{
	...
	if ($self->interface == EVENT_DRIVEN_INTERFACE)
	{
	...
	else
	{
		...
		{
			my $readable_mask = '';
			vec($readable_mask, fileno $self->socket->handle, 1) =1;
			my $ready = select(
		...
		while (1)
		{
			...
			return $self->raise_error(
				...
				and not $self->_incoming_response);
		...
		{
			my $done;
			my $bytes_read = $self->_read_message_header(\$done)
				or return;
		...
		{
			my $done;
			my $bytes_read = $self->_read_message_body(\$done)
				or return;
...
sub send_response
{
	...
	else
	{
		...
		$self->_write_to_socket($response->as_string)
			or return $self->raise_error(
				sprintf("Couldn't send response to " .
				        "server: %s.",
...
sub presentation_uri
{
	my $self = shift;

	$self->{presentation_uri} = shift if @_;

	return $self->{presentation_uri};
}
...
sub is_event_driven
{
	return 1 if (shift->rtsp->interface == EVENT_DRIVEN_INTERFACE);
}
...
sub is_procedural
{
	return 1 if (shift->rtsp->interface == PROCEDURAL_INTERFACE);
}
...
sub _non_blocking_initialization
{
	...
	$self->socket->non_blocking_connect(
		...
		sub {
			$self->state(INITIALIZED);
			$self->succeed;
			$self->socket->register_writable(
				sub { $self->_dispatch_pending_messages }
			);
			$self->socket->register_readable(
...
sub _dispatch_pending_messages
{
	...
	# first, check for timeouts in any active
	...
	if ($self->pipelining)
	{
		while (my $request = shift @{ $self->{_pending_requests} })
		{
			$self->_dispatch_request($request) or last;
		}
	}
	else
	{
...
sub _dispatch_request
{
	...
	unless ($bytes_written)
	{
		...
		$failed_request->fail($self) if $self->is_event_driven;
...
sub _process_incoming_message
{
	...
	if ($self->_received_start_line)
	{
		...
		{
			$self->_read_message_body(\$done) or return;
			if ($done)
			{
				if ($self->state == RECEIVING_RESPONSE)
				{
					$self->_incoming_response->succeed(
		...
		{
			$self->_read_message_header(\$done) or return;
			$slef->_received_message_header(1) if $done;
		}
	}
	else
	{
		$self->_read_start_line(\$done) or return;
		$slef->_received_start_line(1) if $done;
	}
}
...
sub _read_start_line
{
	...
	unless ($bytes_read)
	{
		...
		if ($self->state == RECEIVING_RESPONE)
		{
			my $failed_request = shift @{$self->{_active_requests}};
	...
	if (index($self->_incoming_start_line, "\015\012"))
	{
		...
		{
			(my $version) = $tokens[2] =~ /$version_pattern/;
...
sub _read_message_header
{
	...
	{
		$self->raise_error(
			"Couldn't read the status/request line of " .
			"the server's message: %s.",
			$self->socket->network_error
		);
	...
	if ($self->_incoming_message_header =~ /\015\012[^ \t]/)
	{
		# unfold the possibly folded header
		my ($header, $remainder) =
		...
		{
			# put the restr back, we're not ready for it yet:
...
sub _read_message_body
{
	...
	$$is_done = 1 if ($self->_message_body_bytes_read >= $content_length);
