...
# we export them so the the socket state can be checked and set if needed:
@EXPORT = qw(
	DISCONNECTED
	CONNECTABLE
	CONNECTING
...
# EWOULDBLOCK constants even on Windows (in addition to EINTR):
BEGIN
{
	...
	{
		require Errno;
		import  Errno qw(EINTR EWOULDBLOCK EINPROGRESS);
	}
}
...
sub non_blocking_connect
{
	...
	$self->_make_socket_non_blocking
	...
	unless ($rc or $! == EINPROGRESS or $! == EWOULDBLOCK)
	{
		...
		$self->network_error(remove_package_prefix($@));
...
sub blocking_connect
{
	...
	$self->_make_socket_non_blocking or return;
	...
	{
		$self->state(DISCONNECTED);
		return $self->network_error(remove_package_prefix($@));
	}
...
sub non_blocking_read
{
	...
	my $bytes_read = $self->_system_read(@_);
...
sub non_blocking_write
{
	...
	my $bytes_written = $self->_system_write(@_);
...
#
# 	Purpose
# 		This method returns the IO::Socket::INET filehandle reference
# 		for you to manipulate directly.
#
# 	Parameters
# 		None.
#
...
#
# 	Purpose
# 		This method returns the time() the socket was last active.
#
# 	Parameters
# 		None.
#
